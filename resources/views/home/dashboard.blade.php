                                ...
                                <ul class="nav nav-pills custom-nav nav-justified" role="tablist">
                                    ...
                                    @foreach ($services as $service)
                                        <li class="nav-item" role="presentation">
                                            <button class="nav-link @if ($loop->iteration == 1) active @endif" id="{{ str_replace(' ', '', $service->name) }}-tab" data-bs-toggle="pill"
                                                data-bs-target="#{{ str_replace(' ', '', $service->name) }}" type="button" role="tab" aria-controls="{{ str_replace(' ', '', $service->name) }}" aria-selected="true">{{ $service->name }}</button>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                                                        ...
                                                        <tbody>
                                                            @foreach ($service->services as $ser)
                                                                <tr>
                                                                    ...
                                                                    <td>{{ $data->where('main_service_id', $service->id)->where('service_id', $ser->id)->whereNotIn('status', ['4', '5'])->where('created_at', '<=', date('Y-m-d', strtotime('-4 days')))->where('created_at', '>=', date('Y-m-d', strtotime('-7 days')))->count() }}
                                                                    </td>

                                                                    <td>{{ $data->where('main_service_id', $service->id)->where('service_id', $ser->id)->whereNotIn('status', ['4', '5'])->where('created_at', '<=', date('Y-m-d', strtotime('-8 days')))->where('created_at', '>=', date('Y-m-d', strtotime('-10 days')))->count() }}
                                                                    </td>

                                                                    <td>{{ $data->where('main_service_id', $service->id)->where('service_id', $ser->id)->whereNotIn('status', ['4', '5'])->where('created_at', '<', date('Y-m-d', strtotime('-10 days')))->count() }}
                                                                    </td>
